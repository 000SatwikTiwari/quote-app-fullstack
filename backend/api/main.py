...
def discover_users(
    ...
    for u in users.aggregate(pipeline):
        u["user_id"] = str(u.pop("_id", "")) if "_id" in u else ""
        users_list.append(u)
...
def get_feed(
    ...
    for p in posts:
        ...
        result.append({
            ...
            "email": p["email"],
            "user_id": str(users.find_one({"email": p["email"]}).get("_id", "")) if users.find_one({"email": p["email"]}) else "",
...
def my_followers(skip: int = 0, limit: int = 20, user=Depends(get_current_user)):
    followers_data = list(follows.find({"following": user["email"]}).skip(skip).limit(limit))
    result = []
    for f in followers_data:
        follower_user = users.find_one({"email": f["follower"]})
        if follower_user:
            result.append({
                "user_id": str(follower_user["_id"]),
...
def my_following(skip: int = 0, limit: int = 20, user=Depends(get_current_user)):
    following_data = list(follows.find({"follower": user["email"]}).skip(skip).limit(limit))
    result = []
...
def get_user_profile(user_id: str, user=Depends(get_current_user)):
    try:
        obj_id = ObjectId(user_id)
    except:
        raise HTTPException(400, "Invalid User ID")
        
    target_user = users.find_one({"_id": obj_id})
    if not target_user:
        raise HTTPException(404, "User not found")
        
    # Get quotes for this user
    user_quotes = list(quotes.find({"email": target_user["email"]}).sort("created_at", -1))
    formatted_quotes = []
    for q in user_quotes:
        q_id = str(q["_id"])
        comments_count = comments.count_documents({"quote_id": q_id})
        formatted_quotes.append({
            "quote_id": q_id,
            "content": q["content"],
            "posted_by": target_user["full_name"],
...
def get_user_followers(user_id: str, skip: int = 0, limit: int = 20, user=Depends(get_current_user)):
    try:
        obj_id = ObjectId(user_id)
    except:
        raise HTTPException(400, "Invalid User ID")
    ...
    followers_data = list(follows.find({"following": target_user["email"]}).skip(skip).limit(limit))
    result = []
    for f in followers_data:
        follower_user = users.find_one({"email": f["follower"]})
        if follower_user:
...
def get_user_following(user_id: str, skip: int = 0, limit: int = 20, user=Depends(get_current_user)):
    try:
        obj_id = ObjectId(user_id)
    except:
        raise HTTPException(400, "Invalid User ID")
    ...
    following_data = list(follows.find({"follower": target_user["email"]}).skip(skip).limit(limit))
    result = []
    for f in following_data:
        following_user = users.find_one({"email": f["following"]})
        if following_user:
